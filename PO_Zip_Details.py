...
COLUMNS = [
    ...
    "Inco", "BL/AWB No", "BL/AWB Date", "Container No.",
    "Booked Ship ETD", "Booked Ship ETA", "ETD", "ETA",
    "I/H Plan Month", "I/H Date", "I/H Status",
]
...
# ========= FETCH TRANSIT LINES ==========
def fetch_po_zip_details(company_id, cname):
    ctx = {
    ...
    # Step 1: Fetch transit.line records (company 1 / Zipper)
    line_spec = {
        "transit_id":     {},
    ...
    # Step 2: Fetch purchase.order for PO Apprvd Stat / P Cat / P Type
    ...
    # Step 3: Fetch transit.model headers for remaining fields (incl. container_no)
    def _tid(rec):
    ...
    def map_record(rec):
        ...
        return {
            ...
            "BL/AWB No":       rec.get("bl_number") or "",
            "BL/AWB Date":     transit.get("bl_date") or "",
            "Container No.":   transit.get("container_no") or "",
            ...
            "I/H Date":        transit.get("grn_date") or "",
...
if __name__ == "__main__":
    ...
    if switch_company(company_id):
        records = fetch_po_zip_details(company_id, cname)

        if records:
            df = pd.DataFrame(records, columns=COLUMNS)
            output_file = f"po_zip_details_{today.isoformat()}.xlsx"
